...
/**
 * Full-screen multi-select region picker. Mirrors iOS
 * `MultiSelectRegionPicker` (SpecTechIOS/Scene/Tabs/Marketplace/Filters/MarketplaceFilterSheet.swift)
 * — same data source ([RussianRegions]), same searchable list with checkmarks,
 * same bottom Reset action, same Done confirmation that commits the staged
 * selection back to the caller.
 *
 * Uses [Dialog] rather than a nested [ModalBottomSheet] because the host
 * already lives inside one — nested sheets are jank-prone in Compose.
 *
 * The internal selection ([staged]) only commits when the user taps Done, so
 * back-press / outside-tap discards the in-flight edits — matching the iOS
 * behavior where the pushed list only commits via the Done toolbar item.
 */
@Composable
...
private fun MultiRegionPickerContent(
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        // Bottom Reset — mirrors iOS `.safeAreaInset(edge: .bottom)` Reset chip.
        ResetBar(
